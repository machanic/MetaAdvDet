...
def parse_args():
    ...
    parser.add_argument('--test_num_updates', type=int, default=1, help='number of inner gradient updates during testing')
...
def main():
    ...
    if not args.evaluate:
        ...
        param_prefix = "{}_{}@{}@epoch_{}@meta_batch_size_{}@way_{}@shot_{}@num_query_{}@num_updates_{}@lr_{}@inner_lr_{}".format(
            args.dataset,
        ...
        learner = MetaLearner(args.dataset, args.num_classes, args.meta_batch_size, args.meta_lr, args.inner_lr,
        ...
        report_result = {}
        for model_path in glob.glob("{}/train_pytorch_model/MAML@*".format(PY_ROOT)):
            model_path = "{}/train_pytorch_model/{}".format(PY_ROOT, "MAML@SVHN_TRAIN_ALL_TEST_ALL@conv4@epoch_40@meta_batch_size_10@way_5@shot_1@num_query_15@num_updates_5@lr_0.001@inner_lr_0.01.pth.tar")

            ma_prefix = extract_param_prefix.match(model_path)
            param_prefix = ma_prefix.group(1)
            ma = extract_pattern.match(model_path)
            dataset = ma.group(1)
            args.task_dump_path = "{}/{}/{}".format(args.task_dump_path, args.split_protocol, dataset)
            ...
            #     continue
            learner = MetaLearner(dataset, num_classes, meta_batch_size, meta_lr, inner_lr, epoch, args.test_num_updates, LOAD_TASK_MODE.LOAD,
                                  args.task_dump_path, split_protocol, arch, args.tot_num_tasks, num_support, num_query,param_prefix)
            learner.network.load_state_dict(checkpoint['state_dict'], strict=True)
            learner.opt.load_state_dict(checkpoint['optimizer'])
            result_json = learner.test_task_accuracy()
            report_result[param_prefix] = result_json

        with open(args.task_dump_path + "/report.json", "w") as file_obj:
