...
from traditional_deep_learning.resnet_18.npy_dataset import NpyDataset
...
def resize_image(input_image, random_aspect=False):
    ...
    if random_aspect:
        ...
        height = tf.where(ratio_flag, tf.cast(RESIZE_SIZE * height_orig / width_orig * aspect_ratio, tf.int32),
                          RESIZE_SIZE)
        width = tf.where(ratio_flag, RESIZE_SIZE,
                         tf.cast(RESIZE_SIZE * width_orig / height_orig * aspect_ratio, tf.int32))
    else:
...
def inputs_base(dataset_root, txt_fpath, batch_size, shuffle=False, num_threads=60, num_sets=1, center_crop=False):
    ...
    train_npy_dataset = NpyDataset(dataset_root, txt_fpath, "train")
    for i in range(num_sets):
        ...
        dataset_set = tf.data.Dataset.from_generator(train_npy_dataset.get_generator(), (tf.float32, tf.int32), (tf.TensorShape([IMAGE_HEIGHT, IMAGE_WIDTH, 3]), tf.TensorShape([])))
        dataset_set = dataset_set.batch(batch_size)
