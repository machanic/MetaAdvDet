...
class MAML(nn.Module):
    def __init__(self, model, dim_input, dim_output, meta_lr,
                 num_updates, base_num_filters, two_classification, meta_batch_size,
    ...
    def forward(self, x_support, label_support, x_query, label_query, positive_label, iteration, stage):
        ...
        output_querys = [[]] * self.num_updates
        losses_query = [[]] * self.num_updates
        accuracies_query = [[]] * self.num_updates
        ...
        def task_metalearn(x_support,x_query, label_support, label_query, positive_label):
            ...
            for j in range(self.num_updates - 1):
                loss = self.loss_func(self.model(x_support), label_support)
